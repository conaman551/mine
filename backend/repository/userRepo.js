...
async function createUserOld(name, email, password, phonenumber, biography) {
  ...
  const response = await db.client.query(query, requirements)
  return response
}
...
const createUser = async (email,name) => {
    ...
    try {
        const result = await db.client.query(`INSERT INTO user ("name", "email") VALUES ($1, $2) RETURNING *`,reqs)
...
async function deleteUser(UID) {
 ...
 // let randomnumber = generateNumber()
  ...
  await db.client.query(query,requirements)
}
...
async function getOauthUserID(email) {
  ...
  const resp = await db.client.query(query, requirements);
  return resp[0];
}
...
async function addAppleUser(name,email) {
  ...
  let resp = await db.client.query(query1, requirements1)
  return resp[0].UID
}
...
async function getAppleEmail(appleid) {
  ...
  const user = await db.client.query(query,requirements1)
  if (user.length !== 0) {
    return user[0].email;
  } 
  else {
...
async function getFirebaseToken(UID) {
  ...
  const user = await db.client.query(query)
  return user[0];
}
...
async function getCompletedRides(UID) {
  ...
  const response = await db.client.query(query)
  return response
}
